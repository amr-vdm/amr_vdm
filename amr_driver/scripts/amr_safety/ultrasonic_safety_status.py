...
class UltrasonicSafety:
    ...
    def __init__(self):
        ...
        self.turn_off_ultrasonic_safety_ = True
        ...
        # Publishers:
        self.ultrasonic_safety_status_pub_ = rospy.Publisher(
            "ultrasonic_safety_status", SafetyStatusStamped, queue_size=5
        )
        # Subscribers:
        ...
        while not rospy.is_shutdown():
            if not self.is_running_ or self.turn_off_ultrasonic_safety_:
                ...
                if self.safety_zone_type_ == SafetyZone.SMALL_ZONE:
                    if (
                        self.left_ultrasonic_range_ <= self.small_distance_
                    ...
                    if (
                        self.left_ultrasonic_range_ <= self.default_distance_
                    ...
                    if not self.is_pause_:
                        if self.obstacle_state_ == SafetyStatus.PROTECTED:
                            rospy.logwarn_throttle(
                                1.0, "/ultrasonic_safety_status: Detect obstacle!"
                            )
                        else:
                            rospy.loginfo_throttle(
                                1.0, "/ultrasonic_safety_status: No obstacle in range."
                            )
