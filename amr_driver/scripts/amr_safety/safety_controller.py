...
class SafetyController:
    ...
    def __init__(self):
        ...
        self.is_turn_off_back_ = True
        self.is_turn_off_front_ = True
        self.is_turn_off_ultrasonic_ = True
        self.is_turn_off_front_depth_scan_ = True
        ...
        # Publishers:
        self.protected_field_status_pub_ = rospy.Publisher(
            "status_protected_field", Bool, queue_size=5
        )
        self.speed_at_field_pub_ = rospy.Publisher("/speed_at_field", Float32, queue_size=5)

        # Subscribers:
        rospy.Subscriber(
            "front_scanner_status", SafetyStatusStamped, self.front_scanner_state_callback
        )
        rospy.Subscriber(
            "back_scanner_status", SafetyStatusStamped, self.back_scanner_state_callback
        )
        rospy.Subscriber(
            "front_depth_scanner_status",
            SafetyStatusStamped,
            self.front_depth_scanner_state_callback,
        )
        rospy.Subscriber(
            "ultrasonic_safety_status", SafetyStatusStamped, self.ultrasonic_state_callback
        )
        ...
        rospy.Subscriber(
            "turn_off_ultrasonic_safety", Bool, self.turn_off_ultrasonic_safety_callback
        )
        rospy.Subscriber(
            "turn_off_front_depth_safety", Bool, self.turn_off_front_depth_scan_safety_callback
        )
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if not self.is_running_ or self.is_pause_:
                ...
                if (
                    self.front_scanner_state_ == SafetyStatus.PROTECTED
